...
async def adminFinder(url, session):
    headers = {"User-Agent":random.choice(_useragent_list)}
    async with sem:
        async with session.get(url, headers=headers) as r:
                if r.status == 200:
                    print(f"{g}[{r.status}] {url}")
                elif r.status == 404:
                    print(f"{rd}[{r.status}] {url}")
                elif r.status == 403:
                    print(f"{y}[{r.status}] {url}")
                elif r.status == 301:
                    print(f"{c}[{r.status}] {url}")
                else:
                    print(f"{rs}[{r.status}] {url}")
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        print(banner)
        print("[1] Scan Multiple URLs\n[2] Scan Single URL")
        try:
            op = int(input("[#] Input: "))
        except ValueError:
            print("[x] Error, value is incorrect.")
        ...
        if op == 1:
            ...
            for endpoint in tqdm(endpoints, desc="Processing URLs..."):
                for url in urls:
                        url_parse = urlparse(url)
                        netloc = url_parse.netloc
                        if not url.startswith(("http://", "https://")):
                            fullUrl = f"http://{netloc}{endpoint}"
                        else:
                            scheme = url_parse.scheme
                            fullUrl = f"{scheme}://{netloc}{endpoint}"
                        task.append(adminFinder(fullUrl, session))
            
            await asyncio.gather(*task)
        elif op == 2:
            url_single = input("[#] URL: ")
            for endpoint in endpoints:
                url_single_parse = urlparse(url_single)
                netloc_single_url = url_single_parse.netloc
                if not url_single.startswith(("http://", "https://")):
                    urlFull = f"http://{netloc_single_url}{endpoint}"
                else:
                    scheme = url_single_parse.scheme
                    urlFull = f"{scheme}://{netloc_single_url}{endpoint}"
                task.append(adminFinder(urlFull, session))
